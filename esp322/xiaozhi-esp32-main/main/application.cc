...
void Application::Initialize() {
    ...
    // 👇 重点：TX = GPIO21
    uart_set_pin(UART_NUM_1, 21, -1, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                if (cJSON_IsString(text)) {
                    std::string reply = text->valuestring;
                    ESP_LOGI(TAG, "<< %s", text->valuestring);

                    std::string originalReply = reply;  // 保留原始显示内容
                    ...
                    // 删除英文标点
                    reply.erase(std::remove_if(reply.begin(), reply.end(),
                    ...
                    if (reply.find("TURN ON LIGHT") != std::string::npos) {
                        uartMsg = "TURN ON LIGHT\n";
                        needSend = true;
                    } else if (reply.find("TURN OFF LIGHT") != std::string::npos) {
                        uartMsg = "TURN OFF LIGHT\n";
                        needSend = true;
                    } else if (reply.find("TURN OFF AC") != std::string::npos) {
                        uartMsg = "TURN OFF AC\n";
                        needSend = true;
                    } else if (reply.find("SET AC ON") != std::string::npos) {
                        int temp = 25;  // 默认温度
                    ...
                    // ================= 串口发送 =================
                    if (needSend) {
                        uart_write_bytes(UART_NUM_1, uartMsg.c_str(), uartMsg.length());
                        ESP_LOGI(TAG, "UART Sent: %s", uartMsg.c_str());
                    }

                    // 显示原始文本（不影响聊天效果）
                    Schedule([display, originalReply]() {
                        display->SetChatMessage("assistant", originalReply.c_str());
                    });
                }
            }
            ...
            /*
                ...
                if (cJSON_IsString(text)) {
                    std::string reply = text->valuestring;
                    std::string originalReply = reply;
                    ESP_LOGI(TAG, "<< %s", text->valuestring);
                    ...
                    if (reply.find("TURN ON LIGHT") != std::string::npos) {
                        uartMsg = "TURN ON LIGHT\n";
                        needSend = true;
                    } else if (reply == "TURN OFF LIGHT") {
                        uartMsg = "TURN OFF LIGHT\n";
                        needSend = true;
                    } else if (reply == "TURN OFF AC") {
                        uartMsg = "TURN OFF AC\n";
                        needSend = true;
                    } else if (reply.find("SET AC ON") == 0) {
                        // 解析温度
                        int temp = 25;  // 默认温度

                        size_t pos = reply.find_last_of(" ");
                        if (pos != std::string::npos) {
                            std::string tempStr = reply.substr(pos + 1);
                            int parsed = atoi(tempStr.c_str());
            ...
            */
            ...
            auto text = cJSON_GetObjectItem(root, "text");
            if (cJSON_IsString(text)) {
                std::string recognized = text->valuestring;

                // ================= 串口发送 =================
                // std::string uartMsg = recognized + "\n";
                // uart_write_bytes(UART_NUM_1, uartMsg.c_str(), uartMsg.length());
                // ============================================

                Schedule([display, recognized]() {
                    display->SetChatMessage("user", recognized.c_str());
                });
            }
